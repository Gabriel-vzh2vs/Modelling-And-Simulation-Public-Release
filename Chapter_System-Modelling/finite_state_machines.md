...
To make this concept tangible, we will use a robot-assisted search-and-rescue mission as a running example throughout this chapter. Imagine a team deployed in a hostile environment containing an unknown number of victims. This team consists of a medic, an Unmanned Aerial Vehicle (UAV), and several Unmanned Ground Vehicles (UGVs). The goal is to prioritize the treatment of critical cases while minimizing execution time and energy use. We will specifically focus on modeling the behavior of the UGV using state machines to handle its complex decision-making, such as when to explore, when to report data, and when to triage victims.
