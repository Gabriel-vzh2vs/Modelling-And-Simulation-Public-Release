...
Consider the case with three urns labeled I, II and III containing
silver and gold coins as in Table {ref}`tab:urns`.
...
This section is mostly based on Chapter 4 of {cite}`Law:13` and
{cite}`Taylor:84`, and covers the following topics in quick
succession.
...
Note that we sometimes abbreviate random variable as ``r.v.''.
...
function (subject to certain conditions that you will
find described in Section [insert-ref])

\begin{equation*}
X \colon \Omega \longrightarrow \mathbb{R}
\end{equation*}

It is customary to use uppercase letter to denote random variables such as
$X$, $Y$, and $Z$. The random variable $X$ thus assigns to each sample
point $\omega \in \Omega$ a value $X(\omega)$.

:::

:::{prf:example} {cite}`Law:13`

In this experiment we roll a pair of normal dice. The sample space in
this case is
...
__indicator random variable__.

:::

:::{prf:example} the coin-toss experiment {cite}`Taylor:84`

For the coin toss experiment, we define three random variable. We let
...
```{table} The random variables in the coin tossing experiment.
...
(sec:distribution_function)=
### Distribution functions ###

The __distribution function__, or cumulative distribution function,
abbreviated cdf, of a real-valued random variable~$X$ over a sample space
$\Omega$ is the function $F \colon \mathbb{R} \longrightarrow [0,1]$
defined by
...
If there are multiple random variables in play, we may write $F_X$ or $F_Y$ to avoid ambiguity.


__Discrete- and continuous random variables__

A random variable is a __discrete random variable__ if there is a
finite or denumerable set of distinct values $x_1$, $x_2$, $\ldots$
such that

\begin{equation*}
  a_i = \Pr( X = x_i ) > 0
    \quad
  \text{for $i=1,2,\ldots$ and}\quad \sum_i a_1 = 1 \;.
\end{equation*}

In this case, the function $p$ defined by $p(x_i) = a_i$ for
$i=1,2,\ldots$ is the __probability mass function for $X$__. The
distribution function for $X$ is given by

\begin{equation*}
F(x) = \sum_{x_i \le x} p(x_i) \;.
\end{equation*}

:::{prf:example} {cite}`Law:13` Section 4.5

Here $X$ is the discrete random variable given by $x_1 = 1$, $x_2 =
2$, $x_3=3$, $x_4 = 4$ and $p(x_1) = 1/6$, $p(x_2) = 1/3$, $p(x_3) =
1/3$ and $p(x_4) = 1/6$.

__For the reader:__ draw the graph of (a) the probability mass
function and (b) the distribution function of $X$.
:::

A random variable $X$ for which $\Pr(\{X = x\}) = 0$ for all $x$ is
called a __continuous random variable__. If there is a non-negative
...
then $f(x)$ is called the __probability density function__ for the
...
__For the reader:__ (a) draw $f(x)$ and $F(x)$. You may want to look at
{cite}`Law:13` Table 6.3 of Chapter 6 which contains a wealth of
distributions for common random variables and is a convenient reference.
