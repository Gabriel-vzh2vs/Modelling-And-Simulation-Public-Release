...
Pre-labs marked with "Do" may not be as mathematically rigorous as labs marked with "Read".
...
Which shows the number 42, a integer stored as a floating point number existing in a cell.

### References and Ranges

A cell can also 'reference' another cell, by this, it can include the data stored into 
...
several cells that are referenced (we will get to that later)
...
```{admonition} M/M/1 Example:
   Consider the following, assume that you are analyzing customer service calls at a help desk where calls arrive 
   following an exponential distribution with an average rate of 5 calls per hour ($\lambda$ = 5), 
   then w
```
