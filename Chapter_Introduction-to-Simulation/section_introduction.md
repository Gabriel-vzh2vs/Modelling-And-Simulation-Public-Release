...
- C++ source code compiled into an executable that:
  ...
  - May require advanced hardware archictures such as a cluster, and
...
How can we trust simulation output data? This brings us to the premise
of the simulation approach, namely that:

- The model $M$ captures the system $S$ sufficiently well.  The
- mathematical model $M$ is implemented correctly as a simulation
  model.

What is sufficiently well? It is important to realize that the
precursor to modeling and simulation is typically a set of questions
about $S$. The goal of simulation-based analysis is to provide
meaningful answers to those questions. Fundamentally, this involves
the following two steps.

### Validation ###

The process of ensuring that $M$ represent $S$ in a sufficiently
accurate way is referred to as __validation__. We will cover this in
much more detail in {ref}`sec:validation`. However, validation without
a baseline (the set of questions and requirements) is not very
meaningful.

### Verification ###

The second step, ensuring that the model $M$ is correctly mapped to a
simulation model (i.e., implementation), is called
__verification__. As for validation, this can be a highly nontrivial
process, and we will have more to say about that in {ref}`sec:validation`.
...
This book contains a sequence of topics that could form the basis for
an advanced undergraduate or a graduate course in simulation and
modeling. It covers theory and practice. While one may learn a lot
from just studying the theory, it is the authors' opinion that
"getting your hands dirty" implementing models and running simulations
will truly deepen your understanding.

We have strived to make this book self-contained, but given the nature
of the field, we encourage the reader to explore the literature.
