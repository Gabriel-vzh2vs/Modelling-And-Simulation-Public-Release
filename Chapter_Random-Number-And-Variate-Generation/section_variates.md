...
Where each $F_i(x)$ is a Cumulative Distribution Function and $p_i$ represents the weights that
ensure $\sum_i P_i = 1$.
...
does converge into $F(x)$ when (2) is met.
...
P(X \le x) = \sum_{i} P(X \le x | I = i) P(I = i) \\
